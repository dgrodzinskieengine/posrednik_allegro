...
$renews = M('AllegroWebApiShopAuctionRenew')->db->getAssoc("
	SELECT 
		asar_id, shop_id, 
		asar_renew, 
		asar_renew_queue, 
...
/// 1. Kończenie aukcji przed czasem
$aukcjeDoZakonczenia = M('ShopProductFinish')->find("spf_used = 0 AND spf_error < 5 AND insert_timestamp > now() - INTERVAL 24 hour", array("order" => "shop_id, product_id, kit_id"));
...
// 		consoleLog("SELECT auction_id FROM allegro_shop_auction WHERE shop_id = '{$shop_id}' AND user_id = '{$user_id}' AND product_id = '{$aukcjaDoZakonczenia['product_id']}'");
		...
		//TO DO - przeniesc tego foreach do gory przed continue'a (linia 58)-> produkt sie konczy na sklepie, aukcje tego produktu sa w tabeli allegro_shop_auction_renew i sa oznaczone jako do wznowienia
		//ale podczas konczenia aukcji dla produktow danego sklepu wystepuje jakis blad i jest przerywane usuwanie aukcji produktow tego sklepu ale skrypt leci dalej i wznawia aukcje dla ktorych produkt
		//sie skoncyl
...
// die;
	...
	if ($skip_shop[$shop_id][$user_id])
	{
		$aukcjaDoZakonczenia['spf_error'] = (int)$aukcjaDoZakonczenia['spf_error'] + 1;
...
//$shopAuctionRenews = M('AllegroWebApiShopAuctionRenew')->find('asar_renew_queue = 1 AND asar_renew_repeats <= 3', array('order' => 'update_timestamp', 'limit' => 10)); //dbg

//print_r($shopAuctionRenews);die;
...
//	print_r(
//		array(
//		'shop_id' => $shopAuctionRenew['auction_id'],
//		'asar_id' => $shopAuctionRenew['asar_id'],
//		'asar_renew_repeats' => $shopAuctionRenew['asar_renew_repeats']
//		)
//	);
	...
	if ($shopAuction)
	{
		...
		$old_price = (float)$shopAuction['auction_price'];
	}
	...
	if(empty($shop_api[$shop_id])) {
		try{
			$shop_api[$shop_id] = new Core_ShopApi($shopsAR['shop_url'], $shopsAR['shop_name'], $shopsAR['shop_password']);
			$renew_auctions[$shop_id] = $shop_api[$shop_id]->getAuctionRenew($nr_konfiguracji);
		} catch(SoapFault $soapFault) {
			$renew_auctions[$shop_id] = true;
		}
	}


	if(!$renew_auctions[$shop_id])
	{
	...
	try{
		$prod = $shop_api[$shop_id]->getProduct($product_id);
		$for_sale = $prod->{'for_sale'};
		$product_count = $prod->{'quantity'};
		$product_status = $prod->{'status'};
		$price = $prod->{'price'};
		$price_brutto = round($price * ($prod->{'tax_value'} / 100 + 1),2);

//zabezpieczenie przed naglym spadkiem ceny		
		if($old_price > 0 && $price_brutto > 0) {
			if($price_brutto * 100 / $old_price < 40) {
				$price_brutto = $old_price;
			}
		} else {
			if($old_price > 0) {
				$price_brutto = $old_price;
...
// 			$shopAuctionRenew['asar_renew_queue'] = 0;
	...
	$update_price = false;
	try {
		foreach ($shop_api[$shop_id]->__getFunctions() as $function_name) {
			if (preg_match('/string\s+allegroUpdatePrice\s*\(/',$function_name)) {
				$update_price = $shop_api[$shop_id]->allegroUpdatePrice($product_id, $nr_konfiguracji);
				if ($update_price == '[[auto]]') {
					$update_price = true;
				} elseif (is_numeric($update_price)) {
					$price_brutto = $update_price;
					$update_price = true;
				} else {
					$update_price = false;
				}
	...
	foreach($form as $i => $formPos)
	{
		...
		switch ($formPos->{'fid'})
		{
			...
			case 29:
				...
				break;
			case 8:
				if ($update_price && $price_brutto > 0) {
					$formPos->{'fvalue-float'} = $price_brutto;
				}
			case 7:
	...
	try{
		$can_sell = $shop_api[$shop_id]->canSell($values["fid:5"], $product_count, $product_status, $price);
	}
	catch(SoapFault $soapFault)
	{
		$can_sell = true;
...
// 		{
			...
			$ignored_codes = array(
				...
				'ERR_OBLIGATORY_ATTRIB_NOT_SET' => true
			);
			if($soapFault->faultcode == 'ERR_OBLIGATORY_ATTRIB_NOT_SET')
			{
				consoleLog('ERR_OBLIGATORY_ATTRIB_NOT_SET');
			}
