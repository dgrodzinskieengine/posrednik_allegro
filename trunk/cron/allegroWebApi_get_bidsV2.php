...
// 			continue;
		...
		try
		{
			...
			do {
				...
				$userItems = $client->doGetUserItems($user_id, $web_api_code, $country_id, $offset);
				$time_now = time();
	 	//		print_r($userItems);var_dump($userItems->it-time-left);

				foreach($userItems['user-item-list'] as $userItem)
				{
					$auction_id = $userItem->{'it-id'};
					$item_ids[$auction_id] = $userItem;

					$auction = M('AllegroWebApiShopAuction')->first('auction_id = '.$auction_id);
					if($auction)
						{
							$date_stop = date(' Y-m-d H:i:s', $time_now+$userItem->{'it-time-left'});
					...
					if ($liczba_ofert != $userItem->{'it-bid-count'})
					{
	...
	// 			print_r($userItems);
				$offset++;
			} while(count($userItems['user-item-list']) == 25);
	...
	// 					$auctionError = $auctionsError[0];
						...
						$shopAuction['auction_name'] = $item->{'it-name'};

						$price = strtr($item->{'it-buy-now-price'}, ",", ".");
						if ($price <= 0)
							$price = strtr($item->{'it-price'}, ",", ".");
						...
						$shopAuction['date_start'] = date("Y-m-d H:i:s", $item->{'it-starting-time'});
						$shopAuction['date_stop'] = date("Y-m-d H:i:s", time() + $item->{'it-time-left'});
...
consoleLog("Memory usage (AllegroWebApiShopAuction loaded): ".memory_get_usage(true));
	sleep(1); //zeby nie przekroczyc przypadkowo limitu zapytan na sekunde
	$auctionCountr = count($shopAuctions);
	$counter = 1;
	$start_time = microtime();
	$start_time = explode(' ',$start_time);
...
// 						$postbuyformdata['postbuyform_shipment_type'] = $postBuyDatas[0]->{'post-buy-form-shipment-type'};		// allegro wycofało się z tego
						...
						if($postbuyformdata['shipment_id'] == '10022' || $postbuyformdata['shipment_id'] == '20022')
						{
